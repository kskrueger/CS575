...
def color_change(modify_image, corner, rect, color):
    ...
    for row in range(x-5, x + width+5):
        for col in range(y-5, y + height+5):
...
image = cv2.imread('Part_8/p8_search.png')
...
word_pos = search(word_search_array, "LINEAR")
color_word(imagea, corner, letter_position_array, word_pos, (0, 0, 255))

word_pos = search(word_search_array, "ALGEBRA")
color_word(imageb, corner, letter_position_array, word_pos, (255, 0, 0))

cv2.imwrite("p8_colored_a.png", imagea)
cv2.imwrite("p8_colored_b.png", imageb)
cv2.imshow("Colored Word A", imagea)
cv2.imshow("Colored Word B", imageb)
