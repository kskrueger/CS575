...
for image_file in os.listdir('Part_10'):
    if not image_file.endswith(".png"):
        continue
    image = cv2.imread('Part_10/' + image_file)
    cv2.imshow("in " + image_file, image)

    if image_file.startswith("p10_noisy"):
        image = cv2.imread("Part_10/"+image_file)
        ...
        for filename in os.listdir('Letter_Cutouts'):
            ...
            for c in contours:
                ...
                # else: print("Skipped", rows, col, current_letter)

        # color_change(image, img, corner, (x1, y1, w, h), color)

        colors = [[(0, 0, 0) for i in range(cols)] for j in range(rows)]

        dog_words = ["DALMATIAN", "DACHSHUND", "BEAGLE", "PINSCHER", "SHEPHERD", "MASTIFF", "SETTER", "CHIHUAHUA",
                     "BULLDOG", "COLLIE", "HOUND", "CORGI", "TERRIER", "BOXER"]
        fruit_words = ["RASPBERRY", "LIME", "BLACKBERRY", "BLUEBERRY", "WATERMELON", "ORANGE", "BANANA", "PAPAYA",
                       "LEMON", "KIWI", "STRAWBERRY", "GRAPE", "APPLE"]
        school_words = ["GLITTER", "SHARPENER", "HOMEWORK", "JOURNAL", "ERASER", "BLACKBOARD", "NOTEBOOK", "BACKPACK",
                        "CRAYONS", "SCISSORS", "GLUESTICK", "MARKERS", "BOOKS", "CALCULATOR", "COMPASS", "PENS"]

        if image_file.endswith("search1.png"):
            words_to_find = dog_words
        elif image_file.endswith("search2.png"):
            words_to_find = fruit_words
        elif image_file.endswith("search3.png"):
            words_to_find = school_words
        ...
        cv2.imwrite("colored_" + image_file, image)
        cv2.imshow("Colored " + image_file, image)
        # cv2.waitKey(0)
